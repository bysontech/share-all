...
function NotFoundPage() {
  return (
    ...
    >
      <h1 style={{ fontSize: 64, margin: '0 0 16px' }}>404</h1>
      <p style={{ color: '#666' }}>お探しのページは見つかりません。</p>
    </div>
  );
}
  ...
  return (
    <div style={{ maxWidth: 900, margin: '40px auto', padding: '0 16px', fontFamily: 'sans-serif' }}>
      ...
      <h2 style={{ marginTop: 40 }}>
        ルーム一覧
        {loadingRooms && (
          <span style={{ fontSize: 14, fontWeight: 'normal', marginLeft: 8 }}>読み込み中...</span>
        )}
      </h2>
...
// ── Root: /admin/login ──
// 1. Verify admin_entry cookie via /api/admin/entry-check
// 2. If invalid → show 404
// 3. If valid → check admin session → show login form or admin top

export default function AdminLoginPage() {
  const [entryState, setEntryState] = useState<'loading' | 'denied' | 'allowed'>('loading');
  const [authState, setAuthState] = useState<'checking' | 'login' | 'admin'>('checking');

  useEffect(() => {
    adminApi
      .entryCheck()
      .then(() => {
        setEntryState('allowed');
        return adminApi
          .me()
          .then(() => setAuthState('admin'))
          .catch(() => setAuthState('login'));
      })
      .catch(() => setEntryState('denied'));
  }, []);

  if (entryState === 'loading' || (entryState === 'allowed' && authState === 'checking')) {
